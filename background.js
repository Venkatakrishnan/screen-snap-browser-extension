...
chrome.browserAction.onClicked.addListener(function(tab) {
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    var activeTab = currentTab = tabs[0];
    chrome.tabs.sendMessage(activeTab.id, {"message": "before_screenshot"});
...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
	if( request.message === "before_screenshot_hook" ) {
		var filename = getFilename(currentTab.url);
		
		setTimeout(function(){CaptureAPI.captureToFiles(currentTab, filename, displayCaptures, errorHandler, progress, splitnotifier);}, 100);
	}
		...
		const views = chrome.extension.getViews();
		  for (let i = 0; i < views.length; i++) {
			...
			if (view.location.href == viewTabUrl) {
			  view.setScreenshotUrl(request.filename);
...
function $fn(id) { return document.getElementById(id); }
function show(id) { $fn(id).style.display = 'block'; }
function hide(id) { $fn(id).style.display = 'none'; }
...
function _displayCapture(filenames, index) {
	...
	chrome.tabs.sendMessage(currentTab.id, {"message": "clicked_browser_action", "filename": filename});
...
window.CaptureAPI = (function() {
    ...
    function captureToBlobs(tab, callback, errback, progress, splitnotifier) {
        ...
        chrome.tabs.executeScript(tab.id, {file: 'assets/page.js'}, function() {
            if (timedOut) {
